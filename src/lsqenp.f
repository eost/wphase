...
C                  START THE CALCULATION OF THE PTP MATRIX
      ...
      WRITE (26,383)N,K,IP,M,IFP,GAMCR,DEL,FF,T,E,TAU,AL,ZETA
   82 CONTINUE
      DO 86 I=1,K
      G (I) =0.
      DO 86 J=1,K
...
C                  UNDERFLOWS ALSO PLOT X IN COLUMN ZERO
  ...
  175 WRITE (26,404)IP1,IP2
      GO TO 188
  177 WRITE (26,404)IP1,(IBCH,II=1,I1M2),IP2
      GO TO 188
  179 IF (I1M2.GT.0)GO TO 182
  180 WRITE (26,404)(IBCH,II=1,I1M1),IP1,IP2
      GO TO 188
  182 WRITE (26,404)(IBCH,II=1,I1M1),IP1,(IBCH,II=1,I1M2),IP2
      GO TO 188
  184 WS=Y(I)-F
       IF (IFSS3 .LE. 0 .OR. IPRNT.LT. 0) GO TO 188
  187  WRITE (26,401)X(I,1),Y(I),F,WS
  188 WS=WTS(I)*(Y(I)-F)
      PHD=PHD+WS*WS*1.0D0
      I=I+1
      IF (I.LE.N)GO TO 103
...
C                  BS(J) CORRESPONDS TO PHIZ
  ...
  235 WRITE (26,387)PHIZ,SE,XLL,GAMMA,XL
      GO TO 310
  237 WRITE (26,388)PHIZ,SE,XLL,GAMMA,XL
      GO TO 310
  239 IF (IFSS2.EQ.0)GO TO 242
  240 WRITE (26,379)PHIZ,SE,XL
      GO TO 310
  242 WRITE (26,385)PHIZ,SE,XL
      GO TO 310
  244 PHIL=PHI
...
C     WRITE (26,332) J,SA(J)
  ...
  355 WRITE (26,380)(DB(J),J=1,K)
      WRITE (26,381)PHI,XL,GAMMA,XLL
...
C     ..................................................................
  ...
  386 FORMAT(//T12,'X(I,1)',T31,'Y OBS.',T49,'Y PRED.',T68,'DIFF',/)
...
C     TO INITIATE CONLIM
   ...
   21 DO 22 J=1,K
   22 B(J)=BS(J)
      WRITE (26,201)N,K,IP,M,FF,T,E,TAU
      IBKA=2
C                  THIS WILL PRINT THE Y,YHAT,DELTA Y
   ...
   36 WRITE (26,189)PHIZ,SE,XL
      GO TO 39
   38 WRITE(26,190) PHIZ,SE,XL
...
C                  NOW WE HAVE C = A INVERSE
   ...
   50 KST=KST+5
      ...
      KEND=K
   54 DO 55 I=1,K
   55 WRITE (26,196)I,(A(I,J),J=KST,KEND)
   ...
   73 KST=KST+10
      ...
      KEND=K
   77 DO 78 I=1,K
   78 WRITE (26,202)I,(A(I,J),J=KST,KEND)
...
C                  GET T*SE*SQRT(C(I,I))
   ...
   89 HJTD=SQRT(WS*FF)*SA(J)
      ...
      SPL=BS(J)-HJTD
      SPU=BS(J)+HJTD
      WRITE ( 26,200)J,STE,OPL,OPU,SPL,SPU
      GO TO 98
   97 WRITE (26,191)J
   98 CONTINUE
C                  NONLINEAR CONFIDENCE LIMIT
  ...
  167 WRITE (26,196) J, BL, PL, BU, PU
      GO TO 185
  169 WRITE (26,193) J, BU, PU
      GO TO 185
  171 WRITE (26,196)J,BL, PL
      GO TO 185
  173 WRITE (26,191)J
      GO TO 185
  175 WRITE (26,192)J
...
C     REORDERING THE MATRIX
   ...
   54 CONTINUE
   55 RETURN
   56 WRITE (26,57) P(K),Q(K),PIVOT
   57 format(' 0singular matrix   i=',i3,'j=',i3,'pivot=',e16.8/)
      MSING=2
      GO TO 55
      END

      subroutine fcode(y,x,b,f,i)
      complex c1, c2
      dimension y(*),  x(500,*), b(*)
      pi=3.14159265
c     b(1)=gain_factor, b(2)=fc, b(3)=h
c     h, fc  instrument constant to be used for the difference equation
      g=b(1)
      fc=b(2)
      h=b(3)
      wwdt=2.*pi*fw*dt
      ...
      wc=2.*pi*fc
      c1=cmplx(-w**2,0.)
      c2=cmplx(-w**2,0.)+cmplx(0.,2.*h*wc*w)+cmplx(wc**2,0.)
      f=g*cabs(c1/c2)
      f=alog10(g*cabs(c1/c2))
      return
      end
